...
- **Cập nhật gần nhất**: 2026-06-26 (bổ sung entity `Banner` quản lý carousel trang chủ vào Phase 5)
...
- **Bổ sung 2026-06-26** (sau khi Phase 5 đã Done): thêm entity `Banner` để quản lý carousel trang chủ qua Admin Dashboard, thay vì hardcode trong frontend — phát sinh từ việc user hỏi "ảnh carousel sửa bằng code hay admin sửa được". Scope: entity `Banner` (ImageUrl, Title, Subtitle, LinkUrl, DisplayOrder, IsActive) đúng Clean Architecture (Domain → Application → Infrastructure → API), public `GET /api/banners/active` (không cần auth, cho carousel khách hàng) + Admin CRUD + upload ảnh `POST /api/banners/{id}/image` (tái dùng `IFileStorageService`, thêm subfolder `wwwroot/uploads/banners`); migration `AddBanners` đã tạo và áp dụng. Validate `LinkUrl` phải bắt đầu bằng `/` (đường dẫn nội bộ) để admin không tự ý gắn link ngoài. Frontend: `AdminBannersView.vue` (CRUD dạng bảng + modal, ảnh chỉ upload được sau khi đã lưu banner — giống pattern `AdminProductFormView.vue`), thêm route `/admin/banners` + icon `image` trong sidebar; `BannerCarousel.vue` đổi từ slide hardcode sang fetch `getActiveBanners()`, tự ẩn hoàn toàn nếu chưa có banner nào (không hiện khung rỗng). Test mới: `BannerServiceTests` (5 case) + `BannersControllerTests` (5 case, gồm test chặn `LinkUrl` ngoại miền) — `dotnet test` 144 unit + 89 integration, tất cả pass.
